...
//
//  Multithreaded program to control traffic on an imaginary bridge with two lanes.
//
//  Author: Gabriel Augustin on 7/12/23.
//
...
void *vehicle_routine(void *arg) {
    struct vehicle_args *vargs = (struct vehicle_args *)arg;
    int vehicle_id = vargs->vehicle_id;
    double northbound_prob = vargs->northbound_prob;
    
    int vehicle_type = rand() % 2; //0: Car, 1: Van
    int vehicle_direction = (rand() / (float)RAND_MAX) <= northbound_prob ? 0 : 1; //0: Northbound, 1: Southbound
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < num_groups; i++) {
        ...
        for (j = 0; j < num_vehicles; j++) {
            struct vehicle_args *vargs = malloc(sizeof(struct vehicle_args));
            vargs->vehicle_id = vehicle_id++;
            vargs->northbound_prob = northbound_prob;

            pthread_create(&threads[vehicle_id - 2], NULL, vehicle_routine, vargs);
        }
